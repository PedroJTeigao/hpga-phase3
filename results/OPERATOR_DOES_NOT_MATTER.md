...
- other move classes — a real greedy oracle over a richer move set (multi-position edits, structural moves, or a smarter local-search budget than 8 single-position tries) was not tested, and Evidence 4 only shows that *this* move class is weak here, not that no move class could do better;
