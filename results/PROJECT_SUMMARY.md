...
| 11 | On the lattice, diversity did not become a fitness lead | 3.33 vs 3.67 (10 gen), 3.67 vs 4.00 (15 gen, one seed lost) [P3 §6.3, §7.2]; ~1.9–2.1x worse per token [P3 §5, §6.4] | 3 seeds; P3 counts 1 win, 1 loss, 4 ties [P3 §7.3] |
...
- **Contention as the explanation of the flat dispatch curve (a caveat, not a withdrawal).** P2 §7.3.2 now says the flat curve is "consistent with" contention for the shared GPU. Contention was never measured directly: the `pop_size=32` run shows only that removing the structural fan-out ceiling did not restore scaling, and the section's concurrency probe, on tiny 16-token requests, reached 6.98x at K=8 on the same GPU and model [P2 §7.1–7.3]. Treated here as unproven.
...
**Where sources disagree or overstate** (three earlier inconsistencies, in P3 §6.3/§7.3, P3 §8 and the README, were corrected in commit `6ef0675`).
- P1 §4 puts the serial GA phase at ~8.9% of N=1 wall time, while P1 §3's own DIS/GA ratio at N=1 (10.56 ± 0.26) implies 8.65% (computed as 1/(1+10.56)). P1 does not say how the two relate. P2 §6 quotes 8.9%.
- P2 §7.4 still lists dynamic contention among the "two mechanisms responsible", stronger than the "consistent with" now in §7.3.2.
