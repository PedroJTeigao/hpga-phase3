...
## 9. Circles (Phase 3): the diversity gap, the identical-parent hypothesis, and the crossover-validation redesign

*Covers `hpga/circles.py` and `hpga/blackboard.py` -- a new architecture, not
an extension of §8's `hpga/agents.py` (see `circles.py`'s module docstring
for why: §8's three results -- communication didn't pay for itself, the
diversity effect came from the roles not the channel, and passed folds
carried no reasoning the receiver couldn't derive itself -- constrain this
design rather than being inherited machinery). Anchor run:
`experiments/run_circles_smoke.py`, revision 4, `results/raw/circles_smoke_summary_1789603059.json`
-- `PROMPT_STYLE="best"` (§4.3's `position` mutate + `segment` crossover
simultaneously), `genome_length=18`, `pop_size=8`, `n_generations=10`,
`N_CIRCLES=2`, `AGENTS_PER_CIRCLE=2`, seed=0, single sequence
(`make_timing_sequence(length=20, seed=1)`).*

### 9.1 The diversity gap is the finding
...
on than off, by a wide margin, regardless of which operators were broken at
the time:

| run | config | operator health underneath | gens matched | mean off | mean on | **ratio** |
|---|---|---|---|---|---|---|
| `agents_smoke_1788633787` | `hpga/agents.py`, genome_length=8, A=2 (explore+refine) | free-form role prompts, no affordance fix (§8.3: refine is a self-sustaining no-op) | 5/5 | 1.66 | 5.24 | **3.16x** |
...
| **`circles_1789603059` off/on (this run)** | `hpga/circles.py`, genome_length=18, N_CIRCLES=2x2 | `PROMPT_STYLE="best"`: crossover/mutate near-0% fallback (§4.3) -- the *reliable* end of the operator-health range | 10/10 | 5.06 | 11.28 | **2.23x** |
...
well-tested for a single-seed result (§9.4), and it is the load-bearing
finding of this section -- everything below either qualifies it or asks
what it bought.

### 9.2 What doesn't survive: the identical-parent "feedback loop"
...
|---|---|---|---|
| revision 3 (`run_circles_smoke.py` docstring) | crossover fixed (`segment`); mutate still broken (`full`, 88-93% fallback at genome_length=18) | 0.54 vs. 0.20 | **2.7x** |
| *(unretained -- see below)* | an intermediate configuration during revision 4 | reported around 1.65x at the time | *not verifiable* |
| revision 4 / this run (`circles_smoke_summary_1789603059.json`) | both fixed (`best`: `position` mutate + `segment` crossover, ~0% fallback) | 0.200 vs. 0.167 (unweighted mean across 10 generations) | **1.2x** |
...
**The middle point is a known gap, not papered over.** A ~1.65x figure was
reported during revision 4's development, but no run in `results/raw/`
retains the ops log it would have been computed from -- the only files
with real (non-fixture) `identical_parents` data at this project's
timestamps are this run's off/on logs. It is reported here as *reported,
not verified*, and excluded from the trend line for that reason. This is
the same failure mode already on record for this project's results
folder generally (`README.md`: "Not a git repository... Identity...
recorded as SHA-256 file hashes instead" -- a workaround for the same
underlying problem): a number that exists only in a conversation and not
on disk cannot be checked later. Worth fixing going forward -- every run
this section draws a real conclusion from should leave its raw log behind.
...
shrank**, not as a demonstrated mechanism behind §9.1's gap.

### 9.3 The crossover-validation redesign
...
`segment`-style crossover (§4.3) sidesteps the problem structurally rather
than retuning it. The model declares its own cut
(`SEGMENTS: 0-8:1, 9-17:2`) and the child is *built from that declaration
in code*, not produced independently and then compared against it --
matching the declaration is true by construction. What actually needs
validating is the declaration's structural validity (exact coverage,
>=2 segments, both parents used), which `_llm_crossover` now checks instead
of the post-hoc diff-magnitude threshold. No new threshold, no calibration
free parameter -- it uses information already present in the model's own
response and falls back to the deterministic operator on a malformed
declaration, exactly as before. `full`/`diff`-style crossover still has
nothing to validate against but the output itself, so `CROSSOVER_MIN_DIFF`
remains their fallback gate, uncalibrated, exactly as it was (`hpga/operators.py`
lines 94-122).
...
real," which is what the 17.6% false-rejection rate was actually measuring.
...
- **Circles have not won on fitness, in any of four comparisons in this
  ...
  This is a real contrast with §8's `agents.py` result, not a repeat of it:
  there, `nocomm`/`comm` reached fitness 2.0 against `off`'s flat 1.0 --
  the role architecture *did* move fitness. Circles, on the same kind of
  paired comparison, has not, in this run or in the four ways of reading
  it above. §9.1's diversity gap is real and robust; it has not yet been
  shown to buy anything on the objective the GA actually optimizes.
- **Single seed, ten generations, one problem.** Every number in §9.1's
  `circles` row and all of §9.2-9.3 comes from one run: seed=0, one
  `make_timing_sequence` sequence, `n_generations=10`. The five prior
  §9.1 runs add independent seeds/configs for the *diversity-gap*
  finding specifically, which is why that finding is reported with more
  confidence than §9.3's fitness comparison or §9.2's 1.2x point, neither
  of which has been repeated.
- **The raw-data retention gap flagged in §9.2 is a process problem, not
  just a footnote.** If a run's numbers exist only in a conversation and
  never land in `results/raw/`, they can't be checked again later --
  exactly the position this section was in for the 1.65x point. Future
  revisions of this experiment should treat "save the raw log" as part of
  finishing the run, not an optional step.
